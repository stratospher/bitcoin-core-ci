...
class TestFrameworkKey(unittest.TestCase):
    ...
    def test_hkdf_sha256(self):
        ...
        for test_vector in TESTS:
            IKM, salt, info, L, PRK, OKM = test_vector

            IKM = bytearray.fromhex(IKM)
            salt = bytearray.fromhex(salt)
            info = bytearray.fromhex(info)
            PRK = bytearray.fromhex(PRK)
            OKM = bytearray.fromhex(OKM)

            res_prk = hmac_sha256(salt, IKM)
            res_okm = hkdf_sha256(L, IKM, salt, info)
