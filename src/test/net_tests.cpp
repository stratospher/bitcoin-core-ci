...
void assert_bip324_test_vector(uint16_t in_idx,
                               ...
                               const std::string& mid_session_id_hex,
                               const std::string& mid_rekey_salt_hex,
                               ...
                               const std::string& out_ciphertext_endswith_hex)
{
    ...
    BOOST_CHECK_EQUAL(HexStr(session.session_id), mid_session_id_hex);
    BOOST_CHECK_EQUAL(HexStr(session.rekey_salt), mid_rekey_salt_hex);
    ...
    if (in_initiating) {
        ...
        suite = std::make_unique<BIP324CipherSuite>(session.initiator_L, session.initiator_P, session.rekey_salt);
        ...
        suite = std::make_unique<BIP324CipherSuite>(session.responder_L, session.responder_P, session.rekey_salt);
    }
...
BOOST_AUTO_TEST_CASE(bip324_vectors_test)
{
    ...
    for (const auto& vec : tests.getValues()) {
        assert_bip324_test_vector(
            ...
            vec["mid_session_id"].get_str(),
            vec["mid_rekey_salt"].get_str(),
